...
namespace bm
{
  ...
  {
    {
      ...
      config_loaded_cv.wait(config_lock, [this]()
  ...
  int SwitchWContexts::init_objects(std::istream *is, device_id_t dev_id,
  ...
  int SwitchWContexts::init_objects(const std::string &json_path, device_id_t dev_id,
                                    std::shared_ptr<TransportIface> transport)
  {
  ...
  int SwitchWContexts::init_objects_empty(device_id_t dev_id,
                                          std::shared_ptr<TransportIface> transport)
  {
  ...
  int SwitchWContexts::init_from_options_parser(
      ...
      std::unique_ptr<DevMgrIface> my_dev_mgr)
  {
    ...
    else if (parser.use_simbricks)
    {
      struct SimbricksBaseIfParams sb_params;
      SimbricksNetIfDefaultParams(&sb_params);
      sb_params.sync_interval = parser.sync_inverval;
      sb_params.link_latency = parser.link_latency;
      set_dev_mgr_simbricks(device_id, sb_params, parser.sync_eth, transport);
    }
#ifdef BM_NANOMSG_ON
  ...
  int SwitchWContexts::deserialize(std::istream *in)
  {
  ...
  int SwitchWContexts::deserialize_from_file(const std::string &state_dump_path)
  {
  ...
  int SwitchWContexts::swap_requested()
  {
    for (auto &cxt : contexts)
    {
      if (cxt.swap_requested())
        return true;
    }
    return false;
  }
  ...
  int SwitchWContexts::transport_send_probe(uint64_t x) const
  {
    struct msg_t
    {
      char sub_topic[4];
